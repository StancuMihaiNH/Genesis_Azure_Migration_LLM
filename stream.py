...
ANTHROPIC_API_KEY_LIST = key_vault_manager.get_secret(KeyVaultSecretKeys.ANTHROPIC_KEY_LIST).split(",")
...
async def handle_claude_model(callback, chat_history, session_user_question, user_prompt):
...
async def send_message(item: Item) -> AsyncIterable[str]:
    print(item)
    session_memory, chat_history, session_user_question, session_files, session_tags = get_history_question(
        item
    )
    ...
    if item.model == OpenAIModel.GPT_4.value:
        ...
        print(user_prompt)
        async for msg in handle_gpt_4model( callback, chat_history, session_user_question, user_prompt):
            yield msg

    elif item.model == OpenAIModel.CLAUDE_3_OPUS or item.model == OpenAIModel.CLAUDE_3_OPOUS:
        user_prompt = get_prompt(item.model, session_tags, list_files)
        async for msg in handle_claude_model(callback, chat_history, session_user_question, user_prompt):
            yield msg

    elif item.model == OpenAIModel.NH_QA:
        async for msg in handle_nh_qa_model(callback, chat_history, session_user_question):
            yield msg

    elif item.model == OpenAIModel.TITLE:
        user_prompt = get_prompt(item.model, session_tags, list_files)
        print(user_prompt)
